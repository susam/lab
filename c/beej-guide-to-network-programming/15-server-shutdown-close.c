...
int main(int argc, char **argv)
{
    ...
    printf("accept() #1 accepted connection.\n");

    /* Test 1.1: recv()/send() before shutdown fine. */
    ...
    /* Test 1.2: recv()/send() after shutdown. */
    /* Result: recv()/send() successful. */
    bytes = recv(c_sockfd, buffer, 1024, 0);
    ...
    /* Result: accept() error: Bad file descriptor. */
    c_addr_len = sizeof c_addr;
    if ((c_sockfd =
         accept(sockfd, (struct sockaddr *) &c_addr, &c_addr_len)) == -1) {
         printf("accept() #2 error: %s\n", strerror(errno));
         close(sockfd);
         return EXIT_FAILURE;
    }
